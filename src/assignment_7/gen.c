...
typedef enum op{
    ...
    RET, MINUSI, MINUSF, CHK,LDI, LDIB, SWITCH, SWVALUE, SWDEFAULT, // switch 뺌
...
extern BOOLEAN isArrayType(A_TYPE *);
extern BOOLEAN isPointerOrArrayType(A_TYPE *);
extern BOOLEAN isFloatType(A_TYPE *);
...
void gen_literal_table(){ // 리터럴 테이블의 주소와 값 출력
    int i;
...
void gen_error(int i, int ll, char *s )
{
    ...
    switch (i) {
        ...
        case 23:
            ...
            break;
        case 24:
            printf("%s is excluded for code generation\n",s);
            break;
        case 100:
